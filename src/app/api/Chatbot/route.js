...
// The below code is for automation of workflow where in the userdownloads a json file and then sends it to the chatbot 
import { NextRequest, NextResponse } from 'next/server';
import multer from 'multer';
import fs from 'fs';
import axios from 'axios';
...
const API_URL = 'https://api.groq.com/openai/v1/chat/completions'; // Replace with correct Llama API URL
const API_KEY = process.env.CHATBOT_API_KEY;
        ...
        upload.single('file')(req, {}, async (err) => {
            ...
            try {
                ...
                if (file) {
                    ...
                    if (!jsonData.emissionsBreakdown) {
                        return resolve(NextResponse.json({ error: 'Invalid file format' }, { status: 400 }));
                    }

                    const { flightEmissions, fuelEmissions, powerEmissions, shippingEmissions } = jsonData.emissionsBreakdown;
                    message = `Here is the emissions breakdown: Flight - ${flightEmissions} kg, Fuel - ${fuelEmissions} kg, Power - ${powerEmissions} kg, Shipping - ${shippingEmissions} kg. Please provide personalized suggestions to reduce these emissions.`;
                }
